...
GF_Err gf_odf_set_field(GF_Descriptor *desc, char *fieldName, char *val)
{
	...
	case GF_ODF_DCD_TAG:
	{
		...
		if (!stricmp(fieldName, "objectTypeIndication")) {
			...
			if (!ret) {
				...
				else if (!stricmp(val, "Audio")) { dcd->streamType = GF_STREAM_AUDIO; ret = 1; }
				else if (!stricmp(val, "MPEG7")) { dcd->streamType = GF_STREAM_MPEG7; ret = 1; }
				else if (!stricmp(val, "IPMP")) { dcd->streamType = GF_STREAM_IPMP; ret = 1; }
				else if (!stricmp(val, "OCI")) { dcd->streamType = GF_STREAM_OCI; ret = 1; }
				else if (!stricmp(val, "MPEGJ")) { dcd->streamType = GF_STREAM_MPEGJ; ret = 1; }
			}
		}
	...
	case GF_ODF_MUXINFO_TAG:
	{
		...
		else if (!stricmp(fieldName, "frameRate")) { ret = 1; mi->frame_rate = atof(val); }
	}
		break;
	case GF_ODF_DSI_TAG:
	{
	...
	case GF_ODF_IPMP_TAG:
	{
		...
		else if (!stricmp(fieldName, "IPMP_ToolID")) { ret = 1; OD_ParseBin128(val, &ipmp->IPMP_ToolID); }
	...
	case GF_ODF_IPMP_TOOL_TAG:
	{
		...
		if (!stricmp(fieldName, "IPMP_ToolID")) { ret = 1; OD_ParseBin128(val, &it->IPMP_ToolID); }
