...
static int x264_mb_predict_mv_direct16x16_temporal( x264_t *h )
{
    ...
    int offset = 1;
    int yshift = 1;
    int i_mb_4x4, i_mb_8x8, max_i8, step, width, height, i8;
    ...
    for( i8 = 0; i8 < max_i8; i8 += step )
    {
        ...
        int ypart = (SLICE_MBAFF && h->fref[1][0]->field[mb_xy] != MB_INTERLACED) ?
                    ...
                    3*y8;
        int i_part_8x8, i_ref1_ref, i_ref;
...
static ALWAYS_INLINE int x264_mb_predict_mv_direct16x16_spatial( x264_t *h, int b_interlaced )
{
    ...
    int16_t (*l1mv[2])[2];
    int type_col[2], partition_col[2];
    int mb_x, mb_y, mb_xy, i_mb_4x4, i_mb_8x8;
    int max_i8, step, width, height, i8, i_list;
    ...
    mb_y = h->mb.i_mb_y;
    mb_xy = h->mb.i_mb_xy;
    type_col[0] = h->fref[1][0]->mb_type[mb_xy]; type_col[1] = h->fref[1][0]->mb_type[mb_xy];
    partition_col[0] = h->fref[1][0]->mb_partition[mb_xy]; partition_col[1] = h->fref[1][0]->mb_partition[mb_xy];
    h->mb.i_partition = partition_col[0];
    ...
    l1mv[0] = (int16_t (*)[2]) &h->fref[1][0]->mv[0][i_mb_4x4];
    l1mv[1] = (int16_t (*)[2]) &h->fref[1][0]->mv[1][i_mb_4x4];
