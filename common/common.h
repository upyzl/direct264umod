...
static ALWAYS_INLINE float x264_clip3ff( float v, float f_min, float f_max )
{
    return ( (v < f_min) ? f_min : (v > f_max) ? f_max : v );
}
...
extern const float x264_log2_lut[128][2];
...
static ALWAYS_INLINE float x264_log2( uint32_t x )
{
    ...
    uint32_t a = x << lz;
    uint32_t b = a >> 24;
    return (x264_log2_lut-128)[b][0] + (x264_log2_lut-128)[b][1] * (float)((int32_t)a&0xFFFFFF) + x264_log2_lz_lut[lz];
}
