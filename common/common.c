...
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    OPT2("level", "level-idc")
    {
        if( !strcmp(value, "1b") )
            ...
            p->i_level_idc = lrint(10*atof(value));
        else
    ...
    OPT("fps")
    {
        ...
        {
            p->i_fps_num = lrint(atof(value) * 1000);
            p->i_fps_den = 1000;
        }
    }
    OPT2("ref", "frameref")
    ...
    OPT("keyint")
    {
        ...
        if( p->i_keyint_max && p->i_keyint_min > p->i_keyint_max )
            p->i_keyint_min = p->i_keyint_max;
    }
    OPT2("min-keyint", "keyint-min")
    {
        p->i_keyint_min = atoi(value);
        if( p->i_keyint_max && p->i_keyint_max < p->i_keyint_min )
