...
static int set_param( hnd_t handle, x264_param_t *p_param )
{
    ...
    if( p_param->i_fps_num > 0 && !p_param->b_vfr_input )
    {
        p_mkv->frame_duration = (int64_t)p_param->i_fps_den *
                                1000000000LL / p_param->i_fps_num;
    }
    else
    {
    ...
    p_mkv->d_scalefactor = p_param->i_timebase_num * 1e9 / p_param->i_timebase_den;
...
static int write_headers( hnd_t handle, x264_nal_t *p_nal, int i_nal )
{
    ...
    ret = mk_write_header( p_mkv->w, "x264" X264_VERSION, "V_MPEG4/ISO/AVC",
                           avcC, avcC_len, p_mkv->frame_duration, 1000000,
    ...
    if( sei_size && mk_add_frame_data( p_mkv->w, sei, sei_size ) < 0 )
        return -1;
...
static int write_frame( hnd_t handle, uint8_t *p_nalu, int i_size, x264_picture_t *p_picture )
{
    ...
    i_stamp = llrint(p_picture->i_pts * p_mkv->d_scalefactor);
...
static int close_file( hnd_t handle, int64_t largest_pts, int64_t second_largest_pts )
{
    ...
    i_last_delta = llrint((largest_pts - second_largest_pts) * p_mkv->d_scalefactor);
