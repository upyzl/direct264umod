...
static uint64_t flv_dbl2int( double value )
{
...
static void inline flv_put_byte( flv_buffer *c, uint8_t b )
{
    flv_append_data( c, &b, 1 );
}

static void inline flv_put_be32( flv_buffer *c, uint32_t val )
{
    val = endian_fix32(val);
    flv_append_data( c, &val, 4);
}

static void inline flv_put_be64( flv_buffer *c, uint64_t val )
{
    flv_put_be32( c, val >> 32 );
    flv_put_be32( c, val );
}

static void inline flv_put_be16( flv_buffer *c, uint16_t val )
{
    val = endian_fix16(val);
    flv_append_data( c, &val, 2);
}

static void inline flv_put_be24( flv_buffer *c, uint32_t val )
{
    flv_put_be16( c, val >> 8 );
    flv_put_byte( c, val );
}

static void inline flv_put_tag( flv_buffer *c, const char *tag )
{
    while( *tag )
        flv_put_byte( c, *tag++ );
}

static void inline flv_put_amf_string( flv_buffer *c, const char *str )
{
...
static void inline flv_put_amf_double( flv_buffer *c, double d )
{
