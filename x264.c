...
extern int cropleft, croptop, cropright, cropbottom;
extern double lth, cth, minfps;
...
#endif
#if HAVE_SWSCALE
    printf( "libswscale%s license: %s\n", HAVE_LAVF ? "/libavformat" : "", license );
...
#define H2 if(longhelp==2) printf
    ...
    H0( "\n" );
    H0( " -I, --keyint <int or \"infinite\"> Maximum GOP size [%d]\n", defaults->i_keyint_max );
    H2( " -i, --min-keyint <int>     Minimum GOP size [auto]\n" );
    H2( "     --no-scenecut          Disable adaptive I-frame decision\n" );
    H2( "     --scenecut <int>       How aggressively to insert extra I-frames [%d]\n", defaults->i_scenecut_threshold );
    H2( "     --timed-scenecut       Use PTS based scenecut increment\n" );
    ...
    H2( "     --frame-packing <int>  For stereoscopic videos define frame arrangement\n"
        "                              - 0: checkerboard\n"
    ...
    H0( "Ratecontrol:\n" );
    H0( "\n" );
    H1( " -q, --qp <int>             Force constant QP (0-%d, 0=lossless)\n", QP_MAX );
    ...
    H0( "     --dscrop <left:top:right:bottom>  Apply cropping before subtitle rendering\n" );
    ...
    H0( "Filtering:\n" );
    H0( "\n" );
    H0( " --vf, --video-filter <flt0>/<flt1>/... Apply video filtering to the input file\n" );
    H0( "\n" );
...
static struct option long_options[] =
{
    ...
    { "timed-scenecut",    no_argument, NULL, 0 },
    ...
    { "dscrop", required_argument, NULL, OPT_DSCROP},
        ...
        switch( c )
        {
            ...
            case OPT_IVTC:
                ...
                case 'd':
                    ...
                    break;
                default:
                    x264_cli_log( "x264", X264_LOG_ERROR, "unsupported ivtc mode \"%s\"\n", optarg );
                    return -1;
                }
                break;
            case OPT_SUB:
                ...
                break;
            case OPT_DSCROP:
                if (!sscanf(optarg,"%u:%u:%u:%u",&cropleft,&croptop,&cropright,&cropbottom))
                    return -1;
...
#if HAVE_AVS
    {
        extern int b_nv12, b_sub;
        if (vid_filters)
        {
            if (strstr(vid_filters, "resize"))
                b_nv12 = 0; // HACK! yv12 as default input csp since swscale is not optimized for nv12
            if (strstr(vid_filters, "subtitle"))
                b_sub = 0;
        }
...
static int64_t print_status( int64_t i_start, int64_t i_previous, int i_frame, int i_frame_total, int64_t i_file, x264_param_t *param, int64_t last_ts )
{
    ...
    sec = last_ts * param->i_timebase_num / param->i_timebase_den;
    if( last_ts )
    ...
    if( i_frame_total )
    {
        ...
        sprintf( buf, "x264 %d frames (%d:%02d:%02d): %.2f fps, %.2f kb/s", i_frame, sec/3600, sec/60%60, sec%60, fps, bitrate);
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
    ...
    if( !param->b_repeat_headers )
    {
        ...
        FAIL_IF_ERROR2( (i_file = cli_output.write_headers( opt->hout, headers, i_nal )) < 0, "error writing headers to output file\n" );
    }
