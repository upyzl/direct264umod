...
static void x264_frame_dump( x264_t *h )
{
    ...
    frame_size = FRAME_SIZE( h->param.i_height * h->param.i_width * sizeof(pixel) );
...
static int x264_validate_parameters( x264_t *h, int b_open )
{
    ...
    if( !h->param.i_keyint_max )
        h->param.i_keyint_max = lrintf(h->param.i_fps_num * 10.f / h->param.i_fps_den);
    ...
    if( h->param.rc.i_rc_method == X264_RC_CQP )
    {
        ...
        float qp_i = qp_p - 6.f*log2f( h->param.rc.f_ip_factor );
        float qp_b = qp_p + 6.f*log2f( h->param.rc.f_pb_factor );
        h->param.rc.i_qp_min = x264_clip3( (int)(X264_MIN3( qp_p, qp_i, qp_b )), 0, QP_MAX );
        h->param.rc.i_qp_max = x264_clip3( lrint(X264_MAX3( qp_p, qp_i, qp_b ) + .499), 0, QP_MAX );
    ...
    max_slices = (h->param.i_height+((16<<PARAM_INTERLACED)-1))/(16<<PARAM_INTERLACED);
    if( h->param.b_sliced_threads )
    ...
    fps = h->param.i_fps_num > 0 && h->param.i_fps_den > 0 ? (float) h->param.i_fps_num / h->param.i_fps_den : 25.0;
    if( h->param.i_keyint_min == X264_KEYINT_MIN_AUTO )
        h->param.i_keyint_min = lrintf(X264_MIN( h->param.i_keyint_max * .1f, fps ));
    ...
    h->param.rc.i_lookahead = x264_clip3( h->param.rc.i_lookahead, 0, X264_LOOKAHEAD_MAX );
    {
        ...
        int b = lrintf(bufsize*fps);
        h->param.rc.i_lookahead = X264_MIN( h->param.rc.i_lookahead, X264_MAX( h->param.i_keyint_max, b ) );
    }
    ...
    h->param.rc.f_qcompress = x264_clip3ff( h->param.rc.f_qcompress, 0.f, 1.f );
    ...
    h->param.rc.f_fade_compensate = x264_clip3f( h->param.rc.f_fade_compensate, 0, 1 );
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    logs = x264_analyse_prepare_costs( h );
    if( !logs )
        goto fail;
    ...
    h->out.i_bitstream = X264_MAX( 1000000, h->param.i_width * h->param.i_height * 4
        * ( h->param.rc.i_rc_method == X264_RC_ABR ? powf( 0.95f, h->param.rc.i_qp_min )
          : powf( 0.95f, h->param.rc.i_qp_constant ) * X264_MAX( 1, h->param.rc.f_ip_factor )));
...
#undef COPY
    ...
    ret = x264_validate_parameters( h, 0 );
...
static int x264_encoder_encapsulate_nals( x264_t *h, int start )
{
    ...
    necessary_size = nal_size * 3/2 + h->out.i_nal * 4;
    if( h->nal_buffer_size < necessary_size )
    {
        uint8_t *buf = x264_malloc( h->nal_buffer_size = necessary_size * 2 );
        if( !buf )
            return -1;
...
static void x264_weighted_pred_init( x264_t *h )
{
    ...
    for( i_ref = 0; i_ref < (h->i_ref[0] << SLICE_MBAFF); i_ref++ )
        for( i = 0; i < 3; i++ )
...
static void x264_fdec_filter_row( x264_t *h, int mb_y, int b_inloop )
{
    ...
    if( b_deblock )
        for( y = min_y; y < mb_y; y += (1 << SLICE_MBAFF) )
    ...
    if( PARAM_INTERLACED )
        for( p = 0; p < h->fdec->i_plane; p++ )
            for( i = minpix_y>>(h->mb.chroma_v_shift && p); i < maxpix_y>>(h->mb.chroma_v_shift && p); i++ )
...
static int x264_slice_write( x264_t *h )
{
    ...
    while( 1 )
    {
        int total_bits, b_intra, b_skip, mb_size;
...
static int x264_threaded_slices_write( x264_t *h )
{
    ...
    for( i = 0; i < h->param.i_threads; i++ )
    {
        ...
        height = h->mb.i_mb_height >> PARAM_INTERLACED;
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    if( h->fenc->b_keyframe )
    {
        if( h->param.b_repeat_headers && h->fenc->i_frame == 0 && h->param.i_sei_opts )
        {
            /* identify ourself */
            x264_nal_start( h, NAL_SEI, NAL_PRIORITY_DISPOSABLE );
            if( x264_sei_version_write( h, &h->out.bs ) )
...
#ifdef DEBUG_MB_TYPE
{
    static const char mb_chars[] = { 'i', 'i', 'I', 'C', 'P', '8', 'S',
        'D', '<', 'X', 'B', 'X', '>', 'B', 'B', 'B', 'B', '8', 'S' };
    int mb_xy;
    for( mb_xy = 0; mb_xy < h->mb.i_mb_width * h->mb.i_mb_height; mb_xy++ )
    {
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        float f_bitrate = SUM3(h->stat.i_frame_size) / duration / 125.f;
