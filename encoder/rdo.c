...
uint64_t x264_rd_cost_part( x264_t *h, int i_lambda2, int i4, int i_pixel )
{
    uint64_t i_ssd, i_bits;
    int i8 = i4 >> 2;
    int ssd_x, ssd_y, chromapix, chromassd;
    ...
    chromapix = h->luma2chroma_pixel[i_pixel];
    chromassd = ssd_plane( h, chromapix, 1, ssd_x>>CHROMA_H_SHIFT, ssd_y>>CHROMA_V_SHIFT )
                         ...
                         int b_chroma, int dc, int i_coefs, int idx )
{
    ...
    i_last_nnz = h->quantf.coeff_last[ctx_block_cat]( quant_coefs+b_ac )+b_ac;
    ...
    {
        int i_coef, sign_coef, q, abs_level;
...
#define SET_LEVEL(n,l) \
        ...
        sign_coef = coefs[zigzag[i]];
        i_coef = abs( sign_coef );
        q = abs( quant_coefs[i] );
                         ...
                         int b_chroma, int dc, int i_coefs, int idx, int b_8x8 )
{
    ...
    while( 1 )
    {
        ...
        for( i = b_ac; i <= i_last_nnz; i++ )
        {
            int cur_round, round_change, old_coef, new_coef, cur_mask, cur_distortion_delta;
            int64_t cur_score;
            if( !delta_distortion[i] )
                continue;

            /* Set up all the variables for this iteration. */
            cur_round = round_mask ^ (1 << i);
            round_change = (cur_round >> i)&1;
            old_coef = coefs[i];
            new_coef = quant_coefs[round_change][i];
            cur_mask = (coef_mask&~(1 << i))|(!!new_coef << i);
            cur_distortion_delta = delta_distortion[i] * (round_change ? -1 : 1);
            cur_score = cur_distortion_delta;
            coefs[i] = new_coef;
