...
static NOINLINE void x264_weight_cost_init_chroma( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, pixel *dstu, pixel *dstv )
{
    ...
    if( fenc->lowres_mvs[0][ref0_distance][0][0] != 0x7FFF )
    {
        int x, y, mb_xy, pel_offset_x, pel_offset_y;
        x264_frame_expand_border_chroma( h, ref, 1 );
        for( y = 0, mb_xy = 0, pel_offset_y = 0; y < i_lines; y += height, pel_offset_y = y*i_stride )
            for( x = 0, pel_offset_x = 0; x < i_width; x += 8, mb_xy++, pel_offset_x += 8 )
            {
...
static NOINLINE pixel *x264_weight_cost_init_chroma444( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, pixel *dst, int p )
{
    ...
    if( fenc->lowres_mvs[0][ref0_distance][0][0] != 0x7FFF )
    {
        int x, y, mb_xy, pel_offset_y, pel_offset_x;
        x264_frame_expand_border_chroma( h, ref, p );
        for( y = 0, mb_xy = 0, pel_offset_y = 0; y < i_lines; y += 16, pel_offset_y = y*i_stride )
            for( x = 0, pel_offset_x = 0; x < i_width; x += 16, mb_xy++, pel_offset_x += 16 )
            {
...
static int x264_weight_slice_header_cost( x264_t *h, x264_weight_t *w, int b_chroma )
{
    ...
    denom_cost = bs_size_ue( w[0].i_denom ) * (2 - b_chroma);
    ...
    if( w )
    {
        for( y = 0; y < i_lines; y += 8, pixoff = y*i_stride )
            for( x = 0; x < i_width; x += 8, i_mb++, pixoff += 8)
            {
                ...
                cmp = h->pixf.mbcmp[PIXEL_8x8]( buf, 8, &fenc_plane[pixoff], i_stride );
...
static NOINLINE unsigned int x264_weight_cost_chroma( x264_t *h, x264_frame_t *fenc, pixel *ref, x264_weight_t *w )
{
    ...
    if( w )
    {
        for( y = 0; y < i_lines; y += height, pixoff = y*i_stride )
            for( x = 0; x < i_width; x += 8, pixoff += 8 )
            {
...
static NOINLINE unsigned int x264_weight_cost_chroma444( x264_t *h, x264_frame_t *fenc, pixel *ref, x264_weight_t *w, int p )
{
    ...
    if( w )
    {
        for( y = 0; y < i_lines; y += 16, pixoff = y*i_stride )
            for( x = 0; x < i_width; x += 16, pixoff += 16 )
            {
...
void x264_weights_analyse( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead )
{
    ...
    for( plane = 0; plane <= 2 && !( plane && ( !weights[0].weightfn || b_lookahead ) ); plane++ )
    {
        pixel *mcbuf;
        ...
        if( plane )
        {
            ...
            weights[plane].i_scale = x264_clip3( lrintf(guess_scale * 64), 0, 255 );
            ...
            x264_weight_get_h264( lrintf( guess_scale * 128 ), 0, &weights[plane] );
        ...
        for( i_off = start_offset; i_off <= end_offset; i_off++ )
        {
            unsigned int s;
...
{
    int mb_index;
    int fps_factor = lrint( CLIP_DURATION(average_duration) / CLIP_DURATION(frame->f_duration) * 256 );
    float weightdelta = 0.0, strength;
    if( ref0_distance && frame->f_weighted_cost_delta[ref0_distance-1] > 0 )
        weightdelta = (1.0f - frame->f_weighted_cost_delta[ref0_distance-1]);

    /* Allow the strength to be adjusted via qcompress, since the two
     * concepts are very similar. */
    strength = 5.0f * (1.0f - h->param.rc.f_qcompress);
...
static void x264_macroblock_tree_propagate( x264_t *h, x264_frame_t **frames, float average_duration, int p0, int p1, int b, int referenced )
{
    ...
    x264_emms();
    fps_factor = CLIP_DURATION(frames[b]->f_duration) / CLIP_DURATION(average_duration);
...
#define CLIP_ADD(s,x) (s) = X264_MIN((s)+(x),(1<<16)-1)
                        ...
                        int x, y, mbx, mby, idx0, idx1, idx2, idx3;
                        ...
                        x = mvs[list][mb_index][0];
                        y = mvs[list][mb_index][1];
                        mbx = (x>>5)+h->mb.i_mb_x;
                        mby = (y>>5)+h->mb.i_mb_y;
                        idx0 = mbx + mby * h->mb.i_mb_stride;
                        idx1 = idx0 + 1;
                        idx2 = idx0 + h->mb.i_mb_stride;
                        idx3 = idx0 + h->mb.i_mb_stride + 1;
                        x &= 31;
                        y &= 31;
...
static void x264_macroblock_tree( x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int num_frames, int b_intra )
{
    ...
    int bframes = 0;
    float total_duration = 0.0, average_duration;

    x264_emms();
    for( j = 0; j <= num_frames; j++ )
        total_duration += frames[j]->f_duration;
    average_duration = total_duration / (num_frames + 1);
...
static void x264_slicetype_path( x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int length, char (*best_paths)[X264_LOOKAHEAD_MAX+1] )
{
    ...
    for( path = 0; path < num_paths; path++ )
    {
        ...
        cost = x264_slicetype_path_cost( h, a, frames, paths[idx], best_cost );
        if( cost < best_cost )
        {
            best_cost = cost;
            idx ^= 1;
...
static int scenecut_internal( x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int real_scenecut, float factor )
{
    ...
    f_thresh_max = h->param.i_scenecut_threshold * 0.01f;
    /* magic numbers pulled out of thin air */
    f_thresh_min = f_thresh_max * 0.25f;
    ...
    if( i_gop_size <= h->param.i_keyint_min / 4 || h->param.b_intra_refresh )
    {
        ...
        h->lookahead->i_last_min_keyint_pts = frame->i_pts;
    }
    ...
    else if (!h->param.b_timed_scenecut)
    {
        f_bias = f_thresh_min
    ...
    {
        f_bias = f_thresh_min
                 + ( f_thresh_max - f_thresh_min ) * factor
                 * (float)( frame->i_pts - h->lookahead->i_last_min_keyint_pts )
                 / (float)( h->param.i_keyint_max - h->param.i_keyint_min );
    }
...
static int scenecut( x264_t *h, x264_mb_analysis_t *a, x264_frame_t **frames, int p0, int p1, int real_scenecut, int num_frames, int i_max_search, float scalefactor )
{
    /* Only do analysis during a normal scenecut check. */
    if( real_scenecut && h->param.i_bframe )
    {
        int curp0, curp1, i, maxp1;
        int origmaxp1 = p0 + 1;
        ...
        if( h->param.i_bframe_adaptive == X264_B_ADAPT_TRELLIS )
            ...
            origmaxp1++;
        maxp1 = X264_MIN( origmaxp1, num_frames );

        /* Where A and B are scenes: AAAAAABBBAAAAAA
         * If BBB is shorter than (maxp1-p0), it is detected as a flash
         * and not considered a scenecut. */
        for( curp1 = p1; curp1 <= maxp1; curp1++ )
            if( !scenecut_internal( h, a, frames, p0, curp1, 0, scalefactor ) )
        ...
        for( curp0 = p0; curp0 <= maxp1; curp0++ )
            if( origmaxp1 > i_max_search || (curp0 < maxp1 && scenecut_internal( h, a, frames, curp0, maxp1, 0, scalefactor )) )
    ...
    if( !frames[p1]->b_scenecut )
        return 0;
    return scenecut_internal( h, a, frames, p0, p1, real_scenecut, scalefactor );
}
...
void x264_slicetype_analyse( x264_t *h, int keyframe )
{
    ...
    int reset_start;
    int i, j;
    float scalefactor = (float)( h->param.i_fps_num ) * (float)( h->param.i_timebase_num)
                      /((float)( h->param.i_fps_den ) * (float)( h->param.i_timebase_den));
    ...
    if( h->param.i_scenecut_threshold && scenecut( h, &a, frames, 0, 1, 1, orig_num_frames, i_max_search, scalefactor ) )
    {
        frames[1]->i_type = X264_TYPE_I;
        return;
    }
    ...
    if( h->param.i_bframe )
    {
        ...
        for( j = 1; j < num_bframes+1; j++ )
            if( h->param.i_scenecut_threshold && scenecut( h, &a, frames, j, j+1, 0, orig_num_frames, i_max_search, scalefactor ) )
            {
                frames[j]->i_type = X264_TYPE_P;
    ...
    if( !h->param.b_intra_refresh )
        for( i = keyint_limit+1; i <= num_frames; i += h->param.i_keyint_max )
        {
...
void x264_slicetype_decide( x264_t *h )
{
    ...
    for( bframes = 0, brefs = 0;; bframes++ )
    {
        ...
        if( frm->i_type == X264_TYPE_IDR )
        {
            ...
            h->lookahead->i_last_min_keyint_pts = 0;
        }
...
int x264_rc_analyse_slice( x264_t *h )
{
    ...
    {
        int ip_factor = lrintf(256.f * h->param.rc.f_ip_factor); /* fix8 */
        for( y = 0; y < h->mb.i_mb_height; y++ )
        {
