...
int x264_reference_build_list_optimal( x264_t *h )
{
    ...
    for( ref = 1; ref < h->i_ref[0]; ref++ )
    {
        int max = -1;
        int bestref = 1;

        for( i = 1; i < h->i_ref[0]; i++ )
...
void x264_ratecontrol_init_reconfigurable( x264_t *h, int b_init )
{
    ...
    {
        int vbv_buffer_size, vbv_max_bitrate, bufsize;
        /* We don't support changing the ABR bitrate right now,
           so if the stream starts as CBR, keep it CBR. */
        if( rc->b_vbv_min_rate )
            h->param.rc.i_vbv_max_bitrate = h->param.rc.i_bitrate;

        bufsize = lrint(h->param.rc.i_vbv_max_bitrate / rc->fps);
        if( h->param.rc.i_vbv_buffer_size < bufsize )
        {
            h->param.rc.i_vbv_buffer_size = bufsize;
        ...
        {
            int bitrate, bufsize, max_cpb_output_delay, max_dpb_output_delay, max_delay;
            ...
            bitrate = 1000*h->param.rc.i_vbv_max_bitrate;
            bufsize = 1000*h->param.rc.i_vbv_buffer_size;
            ...
            max_cpb_output_delay = lrint(h->param.i_keyint_max * MAX_DURATION * h->sps->vui.i_time_scale / h->sps->vui.i_num_units_in_tick);
            if (max_cpb_output_delay > INT_MAX) max_cpb_output_delay = INT_MAX;
            max_dpb_output_delay = lrint(h->sps->vui.i_max_dec_frame_buffering * MAX_DURATION * h->sps->vui.i_time_scale / h->sps->vui.i_num_units_in_tick);
            max_delay = lrint(90000.0 * (double)h->sps->vui.hrd.i_cpb_size_unscaled / h->sps->vui.hrd.i_bit_rate_unscaled);
        ...
        if( b_init )
        {
            if( h->param.rc.f_vbv_buffer_init > 1. )
                h->param.rc.f_vbv_buffer_init = x264_clip3ff( h->param.rc.f_vbv_buffer_init / h->param.rc.i_vbv_buffer_size, 0, 1 );
            h->param.rc.f_vbv_buffer_init = x264_clip3f( X264_MAX( h->param.rc.f_vbv_buffer_init, rc->buffer_rate / rc->buffer_size ), 0, 1);
            rc->buffer_fill_final = llrint(rc->buffer_size * h->param.rc.f_vbv_buffer_init * h->sps->vui.i_time_scale);
            rc->b_vbv = 1;
...
int x264_ratecontrol_new( x264_t *h )
{
    ...
    if( h->param.i_nal_hrd )
    {
        ...
        uint64_t num = 180000;
        double bits_required;
        x264_reduce_fraction64( &num, &denom );
        rc->hrd_multiply_denom = 180000 / num;

        bits_required = log2( 180000 * (double)(int32_t)h->sps->vui.i_time_scale * h->sps->vui.hrd.i_cpb_size_unscaled / (int64_t)rc->hrd_multiply_denom );
        if( bits_required >= 63 )
        {
...
#define ABR_INIT_QP (( h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.f_rf_constant : 24 ) + QP_BD_OFFSET)
    ...
    rc->qp_constant[SLICE_TYPE_I] = x264_clip3( lrint(h->param.rc.i_qp_constant - rc->ip_offset), 0, QP_MAX );
    rc->qp_constant[SLICE_TYPE_B] = x264_clip3( lrint(h->param.rc.i_qp_constant + rc->pb_offset), 0, QP_MAX );
    h->mb.ip_offset = lrint(rc->ip_offset);
    ...
    else /* CQP */
    {
        ...
        if( zone )
        {
            if( zone->b_force_qp )
                ...
                q -= 6.f*log2f(zone->f_bitrate_factor);
        }
    }
    if( i_force_qp != X264_QP_AUTO )
        q = i_force_qp - 1;

    q = x264_clip3ff( q, h->param.rc.i_qp_min, h->param.rc.i_qp_max );

    rc->qpa_rc =
    rc->qpa_aq = 0;
    rc->qp = x264_clip3( lrintf(q), 0, QP_MAX );
    h->fdec->f_qp_avg_rc =
    h->fdec->f_qp_avg_aq =
    rc->qpm = q;
    if( rce )
...
void x264_ratecontrol_mb( x264_t *h, int bits )
{
    ...
    if( y < h->i_threadslice_end-1 )
    {
        ...
        float qp_max, buffer_left_planned, slice_size_planned, max_frame_error, size_of_other_slices, rc_tol, b1;
        ...
        if( h->param.b_sliced_threads )
        {
            ...
            weight = rc->slice_size_planned / rc->frame_size_planned;
...
{
    x264_emms();
    return x264_clip3( lrintf(h->rc->qpm), h->param.rc.i_qp_min, h->param.rc.i_qp_max );
}
...
int x264_ratecontrol_mb_qp( x264_t *h )
{
    ...
    return x264_clip3( lrintf(qp), h->param.rc.i_qp_min, h->param.rc.i_qp_max );
}
    ...
    if( h->param.rc.b_stat_read )
    {
        if( frame_num >= rc->num_entries )
        {
            ...
            h->param.rc.i_qp_constant = (h->stat.i_frame_count[SLICE_TYPE_P] == 0) ? 24 + QP_BD_OFFSET
                                      : lrint(1 + h->stat.f_frame_qp[SLICE_TYPE_P] / h->stat.i_frame_count[SLICE_TYPE_P]);
            rc->qp_constant[SLICE_TYPE_P] = x264_clip3( h->param.rc.i_qp_constant, 0, QP_MAX );
            rc->qp_constant[SLICE_TYPE_I] = x264_clip3( lrint( qscale2qp( qp2qscale( h->param.rc.i_qp_constant ) / fabs( h->param.rc.f_ip_factor )) ), 0, QP_MAX );
            rc->qp_constant[SLICE_TYPE_B] = x264_clip3( lrint( qscale2qp( qp2qscale( h->param.rc.i_qp_constant ) * fabs( h->param.rc.f_pb_factor )) ), 0, QP_MAX );
...
int x264_ratecontrol_end( x264_t *h, int bits, int *filler )
{
    ...
    if( h->param.rc.b_stat_write )
    {
        ...
        use_old_stats = h->param.rc.b_stat_read && rc->rce->refs > 1;
        for( i = 0; i < (use_old_stats ? rc->rce->refs : h->i_ref[0]); i++ )
        {
        ...
        if( h->param.rc.b_mb_tree && h->fenc->b_kept_as_ref && !h->param.rc.b_stat_read )
        {
            ...
            for( i = 0; i < h->mb.i_mb_count; i++ )
                rc->qp_buffer[0][i] = endian_fix16( lrintf(h->fenc->f_qp_offset[i]*256.0f) );
    ...
    *filler = update_vbv( h, bits );
    ...
    if( h->sps->vui.b_nal_hrd_parameters_present )
    {
        ...
        {
            double cpb_earliest_arrival_time;
            h->fenc->hrd_timing.cpb_removal_time = rc->nrt_first_access_unit + (double)(h->fenc->i_cpb_delay - h->i_cpb_delay_pir_offset) *
                                                   h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;

            cpb_earliest_arrival_time = h->fenc->hrd_timing.cpb_removal_time - (double)rc->initial_cpb_removal_delay / 90000;
        ...
        filler_bits = *filler ? X264_MAX( (FILLER_OVERHEAD - h->param.b_annexb), *filler )*8 : 0;
        // Equation C-6
...
static int update_vbv( x264_t *h, int bits )
{
    ...
    {
        int64_t scale = llrint(h->sps->vui.i_time_scale * 8);
    ...
    else
    {
        ...
        if( rcc->b_2pass )
        {
            ...
            else
            {
                if( h->i_frame < h->i_thread_frames )
                    predicted_bits += llrint(h->i_frame * rcc->bitrate / rcc->fps);
                else
                    predicted_bits += llrint((h->i_thread_frames - 1) * rcc->bitrate / rcc->fps);
            }
        ...
        else /* 1pass ABR */
        {
                ...
                && rcc->last_non_b_pict_type != SLICE_TYPE_I )
            {
                ...
                q /= fabsf( h->param.rc.f_ip_factor );
            }
            else if( h->i_frame > 0 )
            {
                if( h->param.rc.i_rc_method != X264_RC_CRF )
...
static void x264_threads_normalize_predictors( x264_t *h )
{
    ...
    factor = h->rc->frame_size_planned / totalsize;
    for( i = 0; i < h->param.i_threads; i++ )
...
void x264_threads_merge_ratecontrol( x264_t *h )
{
    ...
    for( i = 0; i < h->param.i_threads; i++ )
    {
        ...
        {
            int size = 0, bits, mb_count;
            for( row = t->i_threadslice_start; row < t->i_threadslice_end; row++ )
                size += h->fdec->i_row_satd[row];
            bits = t->stat.frame.i_mv_bits + t->stat.frame.i_tex_bits + t->stat.frame.i_misc_bits;
            mb_count = (t->i_threadslice_end - t->i_threadslice_start) * h->mb.i_mb_width;
...
static double count_expected_bits( x264_t *h )
{
    ...
    for( i = 0; i < rcc->num_entries; i++ )
    {
        ...
        rce->expected_bits = (int64_t)expected_bits;
...
{
    x264_ratecontrol_t *rcc = h->rc;
    uint64_t all_const_bits = 0, all_available_bits;
    int i;
    double timescale = (double)h->sps->vui.i_num_units_in_tick / h->sps->vui.i_time_scale;
    double duration = 0;
    double rate_factor, step_mult, step;
    double qblur = h->param.rc.f_qblur;
    double cplxblur = h->param.rc.f_complexity_blur;
    const int filter_size = lrint(qblur*4) | 1;
    ...
    duration *= timescale;
    all_available_bits = llrint(h->param.rc.i_bitrate * 1000. * duration);
    ...
    if( all_available_bits < all_const_bits)
    {
        x264_log( h, X264_LOG_ERROR, "requested bitrate is too low. estimated minimum is %d kbps\n",
                  lrint(all_const_bits * rcc->fps / (rcc->num_entries * 1000)));
        return -1;
    }
