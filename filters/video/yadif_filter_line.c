 ...
 * x264 port (C) 2012 James Darnley <james.darnley@gmail.com>
 *
...
static void filter_line_c_16bit( struct yadif_context *yctx )
{
    ...
    const uint16_t *prev2 = (uint16_t *)yctx->parity ? prev : cur ;
    const uint16_t *next2 = (uint16_t *)yctx->parity ? cur  : next;
