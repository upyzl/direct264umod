...
static int handle_opts( const char **optlist, char **opts, video_info_t *info, resizer_hnd_t *h )
{
    ...
    if( fittobox )
    {
        double width_units, height_units;
        ...
        csp = x264_cli_get_csp( h->dst_csp );
        width_units = (double)info->height * in_sar_h * out_sar_w;
        height_units = (double)info->width * in_sar_w * out_sar_h;
        ...
        {
            int new_width = lrint( height * height_units / (width_units * csp->mod_width) );
            new_width *= csp->mod_width;
            width = X264_MIN( new_width, width );
        }
        else
        {
            int new_height = lrint( width * width_units / (height_units * csp->mod_height) );
    ...
    else
    {
        ...
        else if( str_sar ) /* sar only -> adjust res */
        {
             ...
             {
                 width = lrint( info->width * height_units / (width_units * csp->mod_width) );
                 width *= csp->mod_width;
             }
             else // SAR got thinner, decrease height
             {
                 height = lrint( info->height * width_units / (height_units * csp->mod_height) );
...
static int x264_init_sws_context( resizer_hnd_t *h )
{
    ...
    src_format = h->scale.pix_fmt;
    src_range  = handle_jpeg( &src_format );
...
static int init( hnd_t *handle, cli_vid_filter_t *filter, video_info_t *info, x264_param_t *param, char *opt_string )
{
    ...
    h = calloc( 1, sizeof(resizer_hnd_t) );
    if( !h )
        return -1;
    if( opts )
    {
    ...
    src_pix_fmt_inv = convert_csp_to_pix_fmt( info->csp ^ X264_CSP_HIGH_DEPTH );
    dst_pix_fmt_inv = convert_csp_to_pix_fmt( h->dst_csp ^ X264_CSP_HIGH_DEPTH );
